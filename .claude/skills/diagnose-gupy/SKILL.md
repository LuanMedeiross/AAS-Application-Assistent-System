...
| **Finalize** | reported `sent` but didn't finalize | we closed the browser before the submit completed (headless/load) | `gupy/apply.py._finalize_outcome`: waits for "Application finalized!" before `sent`; else `None`→`error` |
| **NotAdvanced** | `error` "clicked Finalizar but Gupy didn't confirm" — but the app IS in the candidate's finished list showing "Você não avançou nesse processo" | knockout/eliminatory question answered disqualifyingly → after "Finalizar" Gupy shows the elimination screen, not the success screen. **Submitted but screened-out — a valid terminal state, not our error** (a false negative) | `gupy/apply.py`: `_NOT_ADVANCED_MARKERS` matched **before** `_DONE_MARKERS` (the screen also has "Acompanhar candidatura") in `_finalize_outcome` + `_detect_step` → outcome `not_advanced`; `services.apply_application` maps it to `result="not_advanced"`, `submitted_at`, `status="applied"` (and guards re-open). Reconcile stale `error` records directly |
